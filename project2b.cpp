...
void testHiddenData() {
    ...
    HiddenDataComp comp;
    PQ<HiddenData, HiddenDataComp> pq(comp);
    ...
    assert(eecsPQ.size() == 2);
    eecsPQ.push(HiddenData{2});
    assert(eecsPQ.size() == 3);
    eecsPQ.push(HiddenData{1});
    assert(eecsPQ.size() == 4);
    eecsPQ.push(HiddenData{8});
    assert(eecsPQ.size() == 5);
    assert(eecsPQ.top().data == 8);
    ...
    eecsPQ.pop();
    assert(eecsPQ.size() == 3);  // NOLINT: Explicit test for size == 0
    eecsPQ.pop();
    eecsPQ.pop();
    eecsPQ.pop();
    assert(eecsPQ.empty());
